...
router.get('/stats', async (req, res) => {
    const { issuedCommands } = (await utils.query(`SELECT issued_commands AS issuedCommands FROM bot_data`))[0]
    ...
    res.render('stats', {
        ...
        messages: { logged: utils.formatNumber(rows), size: Number(gb).toFixed(3) },
