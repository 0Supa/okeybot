const got = require('got');
const { tts_auth } = process.env

const client = got.extend({
  prefixUrl: 'https://api.maia.gg/tts',
  throwHttpErrors: false,
  responseType: 'json',
  headers: {
    'Authorization': tts_auth
  }
});

module.exports = {
  google: function (phrase, lang, speed) {
    return new Promise(async (resolve, reject) => {
      if (!phrase) return reject('no phrase specified')

      let options = `google?text=${encodeURIComponent(phrase)}&lang=${encodeURIComponent(lang) || "en"}&speed=${encodeURIComponent(speed) || "1"}`

      const { body: tts } = await client.get(options)

      if (tts.error) return reject(tts.error || "N/A")
      resolve(tts.url)
    });
  }
};