...
module.exports = {
    ...
    updateRedemption: async function (channelId, id, rewardId, status) {
        const { body } = await helix.patch(`channel_points/custom_rewards/redemptions?id=${encodeURIComponent(id)}&broadcaster_id=${encodeURIComponent(channelId)}&reward_id=${encodeURIComponent(rewardId)}`, { status })
    ...
    bulkBan: function (channelId, users, duration, reason) {
        ...
        for (let i = 0; i < l; i++) {
            await helix.post(`/moderation/bans?broadcaster_id=${channelId}&moderator_id=${config.bot.userId}`, chunks[i])
        }
    },
};