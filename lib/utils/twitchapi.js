...
module.exports = {
    ...
    ivrUser: function (user) {
        return new Promise(async (resolve, reject) => {
            try {
                let userData;
                userData = await got(`https://api.ivr.fi/twitch/resolve/${encodeURIComponent(user)}`).json()
                if (!userData.id) userData = await got(`https://api.ivr.fi/twitch/resolve/${encodeURIComponent(user)}?id=true`).json()
                if (!userData.id) return resolve(null)
                resolve(userData)
            } catch (e) {
                reject(e)
            }
