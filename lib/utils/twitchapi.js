...
module.exports = {
    ...
    getUser: async function (user) {
        ...
        let data = await ivr(`user/${user}`);
        if (!data.body.id) {
            data = await ivr(`user/${user}?id=true`)
    ...
    shortLink: async function (url) {
        let link = url
        const res = await got.post(`https://s.lain.la?url=${encodeURIComponent(url)}`, {
            responseType: 'text',
            throwHttpErrors: false
        })
        if (res.ok) link = res.body
