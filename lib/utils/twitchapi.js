...
const helixMod = got.extend({
    prefixUrl: 'https://api.twitch.tv/helix/moderation',
    responseType: 'json',
    throwHttpErrors: false,
    headers: {
        'Authorization': `Bearer ${config.auth.twitch.helix.token}`,
...
const helixChat = got.extend({
    prefixUrl: 'https://api.twitch.tv/helix/chat',
    responseType: 'json',
    throwHttpErrors: false,
    headers: {
        'Authorization': `Bearer ${config.auth.twitch.helix.token}`,
...
module.exports = {
    ...
    clearChat: async function (channelId) {
        helixMod.delete(`chat?broadcaster_id=${channelId}&moderator_id=${config.bot.userId}`)
    },
    unbanUser: async function (channelId, user) {

        helixMod.delete(`bans?broadcaster_id=${channelId}&moderator_id=${config.bot.userId}&user_id=${user}`)
    },
    timeoutUser: async function (channelId, user, duration) {

        helixMod.post(`bans?broadcaster_id=${channelId}&moderator_id=${config.bot.userId}`, {
            json: {
                data: {
                    user_id: user,
                    duration: duration
    ...
    banUser: async function (channelId, user, reason) {
        helixMod.post(`bans?broadcaster_id=${channelId}&moderator_id=${config.bot.userId}`, {
            json: {
                data: {
                    user_id: user,
    ...
    announceMessage: async function (channelId, message, color) {
        helixChat.post(`announcements?broadcaster_id=${channelId}&moderator_id=${config.bot.userId}`, {
            json: {
                message: message,
                color: color
            }
