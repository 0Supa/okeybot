...
const { getUser, helix } = require('./twitchapi.js')
...
const discordNotify = async (event, streamer, userid, data, cooldownActive) => {
    ...
    switch (event) {
        case 'online': {
            const { body } = await helix.get(`channels?broadcaster_id=${user.id}`)
...
module.exports = async (userid, event, data) => {
    const [channel, streamer] = await Promise.all([
        ...
        utils.query(`SELECT online_format, offline_format, title_format, category_format, discord_webhook, discord_message FROM notify_channels WHERE user_id=?`, [userid])
    ])
    ...
    let message = eventMessages[event]
    if (!message) return
    ...
    if (cachedUsers.length) {
        ...
        const dbUsers = await utils.query(`SELECT user_login FROM notify_users WHERE channel_id=?`, [userid])
    ...
    for (const users of output) {
        ...
        if (channel.pajbot_api) message = await banphraseCheck(message, channel.pajbot_api)
