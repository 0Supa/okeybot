...
const discordNotify = async (event, streamer, userid, data) => {
    ...
    switch (event) {
        case 'online': {
            ...
            if (streamer.discord_webhook && !cooldown.has(`${userid}:noPing:online`)) {
...
module.exports = async (userid, event, data) => {
    ...
    if (cooldown.has(`${userid}:noPing:${event}`)) {
    ...
    let input;
    if (users.length) {
        await redis.sadd(`ob:channel:notifyUsers:${userid}`, users)
        input = users.join(' ')
