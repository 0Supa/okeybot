...
async function hook(json) {
  const { data } = got('https://api.twitch.tv/helix/chat/emotes/global', {
    headers: {
  ...
  const emote = data[Math.floor(Math.random() * data.length)];

  await got.post(process.env.webhook_url, { username: emote.name, avatar_url: emote.images.url_4x, ...json });
}