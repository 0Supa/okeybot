...
exports.notify = async (userid, event, data) => {
  ...
  switch (event) {
    ...
    case "title": message = streamer[0].title_format.replace('%DATA%', data || '(none)'); break;
    case "category": message = streamer[0].category_format.replace('%DATA%', data || '(none)'); break;
    default: return;
  }
...
async function discordNotify(event, streamer, userid) {
...
async function hook(json) {
  await got.post(process.env.webhook_url, { json });
}