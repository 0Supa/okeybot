...
exports.notify = async (channel, event, data) => {
  const streamer = await this.query(`SELECT online_format, offline_format, title_format, category_format, login, user_id, discord_webhook, discord_message FROM notify_data WHERE login=?`, [channel])
...
async function discordNotify(streamer) {
  const channel = await twitch.getChannel(streamer.user_id)
