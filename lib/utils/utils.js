...
exports.change = async (userid, value, data, channelData) => {
    ...
    await utils.query(`UPDATE channels SET ?=? WHERE platform_id=?`, [value, data, userid])
}
...
async function discordNotify(event, streamer, userid, data) {
    ...
    const stream = await twitch.getChannel(userid)
    const userLink = `[**${user.display_name}**](https://www.twitch.tv/${user.login})`
    ...
    switch (event) {
        ...
        case 'title': {
            await hook({
                embeds: [
                    {
                        "description": `${userLink} changed his stream title to \`${data.replace('`', '\\`')}\``,
                        "color": 3092790
                    }
                ]
            })
        ...
        case 'category': {
            await hook({
                embeds: [
                    {
                        "description": `${userLink} changed his stream category to \`${data.replace('`', '\\`')}\``,
                        "color": 3092790
                    }
                ]
            })
...
async function hook(json) {
    let emotes;
    const cacheData = await utils.redis.get(`ob:twitch:globalemotes`)

    if (cacheData) {
        emotes = JSON.parse(cacheData)
    } else {
        const { body } = await got('https://api.twitch.tv/helix/chat/emotes/global', {
            responseType: "json",
            headers: {
                'Client-ID': config.auth.twitch.clientId,
                'Authorization': `Bearer ${config.auth.twitch.password}`
            }
        })
        emotes = body.data

        utils.redis.set(`ob:twitch:globalemotes`, JSON.stringify(emotes), "EX", 86400)
    }

    const emote = emotes[Math.floor(Math.random() * emotes.length)];

    await got.post(config.auth.discordWebhook, { json: { username: emote.name, avatar_url: emote.images.url_4x, ...json } });
}