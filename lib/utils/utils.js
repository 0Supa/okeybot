  ...
  switch (event) {
    ...
    case 'offline': {
      await hook({
        embeds: [
          {
            "description": `**[${channel.display_name}](${channel.url})** is now offline`,
            "color": 3092790
          }
        ]
      })
    ...
    case 'title': {
      await hook({
        embeds: [
          {
            "description": `**[${channel.display_name}](${channel.url})** changed his stream title to \`${data.replace('`', '\\`')}\``,
            "color": 3092790
          }
        ]
      })
    ...
    case 'category': {
      await hook({
        embeds: [
          {
            "description": `**[${channel.display_name}](${channel.url})** changed his stream category to \`${data.replace('`', '\\`')}\``,
            "color": 3092790
          }
        ]
      })
...
async function hook(json) {
  ...
  await got.post(process.env.webhook_url, { json: { username: emote.name, avatar_url: emote.images.url_4x, ...json } });
}