...
const songHumanize = humanize.humanizer({
    language: 'song',
    languages: {
        song: {
            h: () => ':',
...
exports.notify = async (userid, event, data) => {
    ...
    let users;
    const cachedUsers = await utils.redis.smembers(`ob:channel:${userid}:notifyUsers`)
    ...
    let input;
    if (users.length) {
        await redis.sadd(`ob:channel:${userid}:notifyUsers`, users)
        input = users.join(' ')
