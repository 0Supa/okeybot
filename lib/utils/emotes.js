...
module.exports = {
    ...
    BTTVemote: function (option, emoteID, userID) {
        return new Promise(async (resolve, reject) => {
            const bttvID = await this.getBTTVid(userID).catch(err => {
                return reject(`couldn't resolve bttv userID (${err})`)
            })

            const emote = await this.getBTTVemote(emoteID).catch(err => {
