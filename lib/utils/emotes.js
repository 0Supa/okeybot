    ...
    getSTVid: function (user) {
        return new Promise(async (resolve, reject) => {
            const { body, statusCode } = await got.post(`https://api.7tv.app/v2/gql`, {
                throwHttpErrors: false,
                responseType: 'json',
                json: { "query": `{user(id: \"${user.replaceAll('"', '\\"')}\") {...FullUser}}fragment FullUser on User {role {id}}` }
            });
            if (statusCode < 200 || statusCode > 299) return reject(`an unexpected error occurred (${statusCode})`)
            if (!body.data.user.role.id) return reject(`user not found`)
            resolve(body.data.user.role.id)
        });
    },
    ...
    STVemote: function (option, emoteID, userID) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                let json;
                if (option === 'add') {
                    if (emote.visibility === 4) return reject("this emote has not been accepted yet")
                    if (emote.visibility !== 0 && emote.status !== 3) return reject("this emote didn't pass the validation")
                ...
                else if (option === 'remove') json = { "query": "mutation RemoveChannelEmote($ch: String!, $em: String!, $re: String!) {RemoveChannelEmote(channel_id: $ch, emote_id: $em, reason: $re) {emote_ids}}", "variables": { "ch": userID, "em": emoteID, "re": "" } }

                const { statusCode } = await got.post(`https://api.7tv.app/v2/gql`, {
                    headers: {
                ...
                if (statusCode < 200 || statusCode > 299) throw `an unexpected error occurred (${statusCode})`;
