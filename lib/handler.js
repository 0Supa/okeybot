    ...
    handle: async function (message) {
        ...
        if (command && !cooldown.has(`${command.name} ${message.user.id}`) && message.text.startsWith(message.prefix)) {
            ...
            try {
                ...
                const commandCooldown = result.cooldown || command.cooldown
                if (commandCooldown && message.user.login !== 'supa8') {
        ...
        if (message.channelName === 'omuljake') {
            ...
            if (message.isAction && msgText.split(' ').some(x => x.includes('donat'))) {
