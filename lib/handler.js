...
module.exports = {
    handle: async function (message) {
        ...
        if (command && !cooldown.has({ command: command.name, user: message.user.id }) && message.text.startsWith(prefix)) {
            ...
            if (command.cooldown && message.user.login !== 'supa8') {
                cooldown.add({ command: command.name, user: message.user.id });
                setTimeout(() => {
                    cooldown.delete({ command: command.name, user: message.user.id });
