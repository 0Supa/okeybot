        ...
        if (msg.user.id !== process.env.userid) {
            ...
            if (msg.channel.login === 'omuljake') {
                if (msg.tags['custom-reward-id'] && msg.tags['custom-reward-id'] === '15459ba8-7536-42f5-9257-6bc47a4df70d') {
                    ...
                    await BTTVemote('remove', currentID).catch(err => {
                        return client.say(msg.channel.login, `${msg.user.name}, couldn't remove previous requested emote, error: ${err}`)
                    })

                    const parsedInput = (new RegExp(/https?:\/*betterttv\.com\/emotes\/([A-Za-z0-9]+)/)).exec(msg.text);
                    if (!parsedInput) return

                    const emoteName = await BTTVemote('add', parsedInput[1]).catch(err => {
                        return client.say(msg.channel.login, `${msg.user.name}, couldn't add emote, error: ${err}`)
                    })

                    await utils.query(`UPDATE jake SET emote_id=?`, [parsedInput[1]])
                    client.say(msg.channel.login, `PagChomp ${msg.user.name} successfully added emote ${emoteName}`)
                    return;
                }
                ...
                const input = msg.text.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "")

                if (input.split(' ').some(x => x.includes('donat'))) {
                ...
                else if (bannedWords.some(word => input.includes(word))) {
                    const messages = (await utils.query(`SELECT COUNT(id) AS entries FROM messages WHERE user_id=? AND channel_id=? LIMIT 10`, [msg.user.id, msg.channel.id]))[0].entries
                    if (messages < 10) client.ban(msg.channel.login, msg.user.login)
                }
