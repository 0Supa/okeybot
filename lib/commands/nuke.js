module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        if (msg.args[0].toLowerCase() === 'exact') {
            const query = await utils.query(`SELECT user_login FROM messages WHERE timestamp > DATE_SUB(NOW(),INTERVAL 30 MINUTE) AND channel_id=? AND message=?`, [msg.channel.id, msg.args.slice(1).join(' ')])
            nuke(query)
        }
        else if (msg.args[0].toLowerCase() === 'like') {
            const query = await utils.query(`SELECT user_login FROM messages WHERE timestamp > DATE_SUB(NOW(),INTERVAL 30 MINUTE) AND channel_id=? AND message LIKE ?`, [msg.channel.id, msg.args.slice(1).join(' ')])
            nuke(query)
        }
        else {
