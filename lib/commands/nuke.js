    ...
    async execute(client, msg, utils) {
        ...
        if (duration === 'ban') {
            for (const target of users)
                client.privmsg(msg.channel.login, `/ban ${target} nuked with RegExp "${regexp[1]}"`)
        } else {
            for (const target of users)
                client.privmsg(msg.channel.login, `/timeout ${target} ${duration} nuked with RegExp "${regexp[1]}"`)
        }
