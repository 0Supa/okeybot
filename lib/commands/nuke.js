...
module.exports = {
    ...
    botRequires: 'mod',
    cooldown: 15,
    usage: "<loopback seconds> <timeout in seconds (0 = ban)> <regex phrase>",
    async execute(client, msg, utils) {
        const usage = `usage: ${msg.prefix}nuke <loopback seconds> <timeout in seconds (0 = ban)> <regex phrase>`
        ...
        if (duration === 'ban') {
            for (const target of users)
                const reason = `banned with RegExp "${regexp[1]}"`
                twitchapi.banUser(msg.channel.id, target, reason)
        } else {
            for (const target of users)
                const reason = `nuked with RegExp "${regexp[1]}"`
                twitchapi.timeoutUser(msg.channel.id, target, duration, reason)
        }
