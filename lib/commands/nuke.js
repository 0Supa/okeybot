module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        const phrase = msg.args.slice(1).join(' ').match(new RegExp('^/(.*?)/([gimsuy]*)$'))
        ...
        for (let i = 0; i < messageCount; i++) {
            ...
            try {
                ...
                commands++
                await client.ban(msg.channel.login, message.user, `nuked with phrase "${phrase[1]}"`)
                if (commands === 60) {
                    commands = 0
                    await sleep(35000)
                }
            } catch (e) {
                errorUsers.push(message.user)
                console.error(e)
            }
        }
