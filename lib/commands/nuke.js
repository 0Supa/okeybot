module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        const phrase = msg.args.slice(1).join(' ')
        const query = await utils.query(`SELECT user_login FROM messages WHERE timestamp > DATE_SUB(NOW(),INTERVAL ? SECOND) AND channel_id=? AND message REGEXP ?`, [interval, msg.channel.id, phrase])
        const messageCount = query.length
        if (!messageCount) return { text: `no messages matched your phrase`, reply: true }
        ...
        for (let i = 0; i < messageCount; i++) {
            const user = query[i].user_login
            if (users.includes(user)) { continue; }

            try {
                users.push(user)
                commands++
                await client.ban(msg.channel.login, user, `nuked with phrase "${phrase}"`)
                if (commands === 60) {
                    commands = 0
                    await sleep(35000)
                }
