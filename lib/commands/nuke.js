...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        let ban
        if (punish === 'ban') {
            ban = (user) => { client.privmsg(msg.channel.login, `/ban ${user} nuked with phrase "${phrase[1]}"`) }
        } else {
            ban = (user) => { client.privmsg(msg.channel.login, `/timeout ${user} ${punish} nuked with phrase "${phrase[1]}"`) }
        }
