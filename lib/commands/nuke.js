...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        const messages = await utils.query(`SELECT user_id AS user, message AS text FROM messages WHERE timestamp > DATE_SUB(NOW(),INTERVAL ? SECOND) AND channel_id=?`, [interval, msg.channel.id])
        ...
        if (punish === 'ban') punish = null

        const errors = await twitch.bulkBan(msg.channel.id, users, punish, `nuked with RegExp "${regexp[1]}"`)

        await utils.redis.set(`ob:channel:nuke:${msg.channel.id}`, JSON.stringify(users), "EX", 86400) // 1 day

        if (errors.length) {
            const paste = await got.post('https://paste.ivr.fi/documents', { body: errors.join('\n') }).json()
            return { text: `☢ Successfully nuked ${users.length - errors.length}/${users.length} users, errors: https://paste.ivr.fi/${paste.key}` }
        }
        return { text: `☢ Successfully nuked ${users.length - errors.length}/${users.length} users` }
    },
};