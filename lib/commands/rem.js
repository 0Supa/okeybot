...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        const tmiFormat = messages.map(m => `@badges=${m.sender.displayBadges.map(b => `${b.setID}/${b.version}`).join()};color=${m.sender.chatColor};display-name=${m.sender.displayName};id=${m.id};room-id=${res.data.channel.id};rm-received-ts=${Date.parse(m.sentAt)} :${m.sender.login}!${m.sender.login}@${m.sender.login}.tmi.twitch.tv PRIVMSG #${channel} :${m.content.text}`).join('\n')

        const paste = await got.post('https://paste.ivr.fi/documents', { body: tmiFormat }).json()
