...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        if (count[0].query) {
            ...
            const emptyConnection = stv.connections.find(conn => !conn.channels.length)
            if (emptyConnection) delete emptyConnection
        ...
        if (!stv.channels.includes(msg.channel.login)) {
            const connection = stv.connections.find(conn => conn.channels.length < 100)
