...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        if (random) {
            ...
            const paste = await got.post('https://paste.ivr.fi/documents', { body: foundMessages.map(message => `(${utils.humanize(message.timestamp)} ago) ${message.user_login}: ${message.text}`).join('\n') }).json()
