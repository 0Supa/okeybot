...
module.exports = {
    name: 'scan',
    description: 'Find messages using a RegExp',
    cooldown: 20,
    async execute(client, msg, utils) {
        ...
        if (random) {
            ...
            const fmt = bruh.map(message => `(${utils.humanize(message.timestamp)} ago) ${message.user_login}: ${message.text}`).join('\n')
            return { text: `[${foundMessages.length}/${messageCount} messages matched] ${sliced ? "first 5000 matched messages: " : ""} ${await paste(fmt, true)}`, reply: true }
        }
    },
};
