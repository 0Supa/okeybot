    ...
    async execute(client, msg, utils) {
        ...
        for (let i = 0; i < messageCount; i++) {
            ...
            if (regex.test(message.text)) {
                if (foundMessagesLength < 5000) foundMessages += `(${utils.humanize(message.timestamp)} ago) ${message.user_login}: ${message.text}\n`
                foundMessagesLength++
            }
        }
        ...
        return { text: `[${foundMessagesLength}/${messageCount} messages matched] https://paste.ivr.fi/raw/${paste.key}${foundMessagesLength > 5000 ? " (only first 5000 found messages)" : ""}`, reply: true }
    },
};