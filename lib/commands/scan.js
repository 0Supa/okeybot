...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        if (random) {
            const message = utils.randArray(messages)
        } else {
            let sliced = false
            if (foundMessages.length > 5000) {
                foundMessages = foundMessages.slice(0, 5000)
                sliced = true
            }

            const paste = await got.post('https://paste.ivr.fi/documents', { body: foundMessages.map(message => `(${utils.humanize(message.timestamp)} ago) ${message.user_login}: ${message.text}\n`) }).json()
