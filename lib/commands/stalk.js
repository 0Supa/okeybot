...
module.exports = {
    ...
    async execute(client, msg, utils) {
        if (!msg.args.length) return { text: 'you need to specify an username', reply: true }
        ...
        return {
            text: `that user's last seen message was sent ${utils.humanize(query[0].timestamp)} ago in ${query[0].channel_login === user ? 'their' : `${query[0].channel_login}'s`} chat: ${query[0].message}`,
            reply: true
        }
    },
};
