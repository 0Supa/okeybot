module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        if (user === msg.user.login) return { text: '4Head', reply: true }
        if (user === process.env.botusername) return { text: "I'm here FeelsDankMan", reply: true }
        ...
        return {
            text: `that user's last seen message was sent ${utils.parseSec(date)} ago in ${query[0].channel_login === user ? 'his' : `${query[0].channel_login}'s`} chat: ${query[0].message}`,
            reply: true
        }
    },
};