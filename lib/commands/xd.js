...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        switch (option) {
            ...
            case "pull": {
                const { stdout } = await exec('git pull').toString().split('\n').filter(Boolean)
                if (stdout.includes('Already up to date.')) return { text: 'no changes detected 🤓☝' }
                return { text: `🤓👉 ${getChanges(stdout) || stdout.join(' | ')}` }
            }
            case "restart": {
                const { stdout } = await exec('git pull').toString().split('\n').filter(Boolean)
                if (stdout.includes('Already up to date.')) await msg.send('ppCircle Restarting without any changes')
                else await msg.send(`ppCircle Restarting 🤓👉 ${getChanges(stdout) || stdout.join(' | ')}`)
