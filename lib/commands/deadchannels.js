                ...
                if (ms > 604800000) { // 1 week
                    ...
                    if (msg.args[0]?.toLowerCase() === 'part') {
                        ...
                        await Promise.all([
                            ...
                            utils.query(`DELETE FROM notify_data WHERE user_id=?`, [channel.id]),
                            utils.query(`DELETE FROM emotes WHERE channel_id=?`, [channel.id]),
                            utils.query(`DELETE FROM 7tv WHERE login=?`, [channel.login]),
                        ]);
