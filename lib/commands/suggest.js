...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        if (msg.commandName === 'addbot') {
            ...
            if (msg.args[0]) {
               const check = await utils.query(`SELECT login FROM channels WHERE login=?`, [msg.args[0]])
               if (check.length) return { text : `I'm already in the channel ${utils.antiping(msg.args[0])}`}
        } else {
            const checkelse = await utils.query(`SELECT login FROM channels WHERE login=?`, [msg.user.login])
             if (checkelse.length) return { text : `I'm already in your channel 4Head` }
        }
            ...
            dink = `new bot request peepoDetective 👉 https://paste.ivr.fi/${paste.key}`
        }
        else {
