module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        const suggestion = (await utils.query(`SELECT id FROM suggestions WHERE text=? AND status=? LIMIT 1`, [text, 'Pending Review']))[0]
        if (suggestion) return { text: `this suggestion already exists (ID: ${suggestion.id})`, reply: true }
        ...
        client.say(process.env.botusername, `@supa8 new suggestion (ID: ${id}) DinkDonk`)
    },
};