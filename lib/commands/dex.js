...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        const definitions = data.definitions.map(def => `(Sursa: ${def.sourceName ?? "N/A"}): ${def.htmlRep.replace(/<[^>]*>?/gm, '')}`)
        return { text: `${redirected ? `[${searchWord} => ${data.word}]` : `[${data.word}]`} ${definitions.join(' | | ')}`, reply: true }
    },
};
