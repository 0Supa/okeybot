    ...
    async execute(client, msg, utils) {
        ...
        switch (option) {
            case "join": {
                ...
                if (!data[0].entries) {
                    const user = await twitchapi.getUser(channelName)
                    if (!user) return { text: "couldn't resolve the user provided" }
                    await utils.query(`INSERT INTO channels (platform_id, login) VALUES (?, ?)`, [user.id, channelName])
            ...
            case "part": {
                ...
                return { text: `successfully parted channel ${channelName}` }
            }
