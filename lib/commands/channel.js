...
const globalSubs = ['video-playback-by-id', 'chatrooms-user-v1']
        ...
        switch (option) {
            case "join": {
                ...
                if (!data[0].entries) {
                    ...
                    try {
                        ...
                        if (!msg.text.split(' ').includes('-silent')) await client.say(user.login, `FeelsOkayMan Successfully joined! You can use "${config.bot.defaultPrefix}help" for the command list. If you need more assistance, use "${config.bot.defaultPrefix}suggest" followed by your question or suggestion.`)
                    } catch (e) {
                        console.error(e)
            ...
            case "part": {
                ...
                await Promise.all([
                    ...
                    utils.redis.del(`ob:channel:clips:${user.id}`),
                    utils.redis.del(`ob:channel:nuke:${user.id}`),
                    utils.query(`DELETE FROM channels WHERE platform_id=?`, [user.id]),
                    utils.query(`DELETE FROM notify_channels WHERE user_id=?`, [user.id]),
                    utils.query(`DELETE FROM emote_rewards WHERE channel_id=?`, [user.id]),
                ])

                await client.say(msg.channel.login, `successfully parted channel ${channel} - ${user.id}, deleting message logs... (this might take a while)`)

                await utils.query(`DELETE FROM messages WHERE channel_id=?`, [user.id])
                return { text: `successfully deleted message logs for ${channel} - ${user.id}` }
            }
