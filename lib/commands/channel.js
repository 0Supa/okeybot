            ...
            case "join": {
                ...
                if (!data[0].entries) {
                    ...
                    try {
                        ...
                        if (!/\b-silent\b/.test(msg.text)) await client.say(user.login, `Successfully joined, if you have any questions use "${config.bot.defaultPrefix}suggest" followed by your question/suggestion | Type "${config.bot.defaultPrefix}help" for the command list BroBalt`)
                    } catch (e) {
                        console.error(e)
