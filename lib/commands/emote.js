    ...
    async execute(client, msg, utils) {
        ...
        if (msg.emotes.length) {
            ...
            data = {
                ...
                tier: subscriptionTier.substring(5),
                setid: emote.setID,
                emoteurl_3x: `https://static-cdn.jtvnw.net/emoticons/v2/${emote.id}/default/dark/3.0`
            }
        } else {
            data = await twitchapi.ivrEmote(emoteIdentifier)
            if (!data) return { text: `emote was not found`, reply: true }
        }

        return { text: `${data.emoteid} - ${data.emotecode} | ${data.channel ? `${data.channel} tier ${data.tier} emote` : "Global Emote"}, setid: ${data.setid} | ${data.emoteurl_3x}`, reply: true }
    },
};