...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        if (cacheData) {
            ...
            const clips = await getClips(msg.channel.login)
            if (!clips) return { text: 'this channel has no clips', reply: true }

            utils.redis.set(`ob:channel:clips:${msg.channel.id}`, JSON.stringify(clips), "EX", 86400)
        }

        const randomClip = utils.randArray(clips)
        return { text: `${randomClip.url} - ${randomClip.title} - Clipped by: ${randomClip.curator.displayName}`, reply: true }
    },
};