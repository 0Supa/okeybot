...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        async function seventv(emote) {
            const { data } = await got.post(`https://api.betterttv.net/3/emotes/shared/search?query=${encodeURIComponent(emote)}&limit=1`, {
                responseType: 'json',
                json: { "query": `{search_emotes(query: ${JSON.stringify(emote).slice(1, -1)}, limit: 1, sortBy: \"popularity\", sortOrder: 0) {id,visibility,owner {id,display_name,role {id,name,color},banned}name}}` }
            })
