...
module.exports = {
    ...
    execute(client, msg, utils) {
        ...
        async function ffz(emote) {
            const { emoticons } = await got(`https://api.frankerfacez.com/v1/emoticons?q=${encodeURIComponent(emote)}&per_page=1&sort=count`, { responseType: 'json' })
        ...
        async function bttv(emote) {
            ...
            const emoticons = await got(`https://api.betterttv.net/3/emotes/shared/search?query=${encodeURIComponent(emote)}&limit=1`, { responseType: 'json' })
