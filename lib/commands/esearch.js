...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        async function ffz(emote) {
            const { emoticons } = await got(`https://api.frankerfacez.com/v1/emoticons?q=${encodeURIComponent(emote)}&per_page=1&sort=count-desc`).json()
        ...
        async function seventv(emote) {
            ...
            if (res.data.emotes.count === 0) return { text: 'no emotes found', reply: true } // No Items Found
            if (res.errors) {
