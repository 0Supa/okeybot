...
const { kick, paste, shortLink } = require('../utils/twitchapi.js')
...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        if (values.json)
            ...
            if (data.livestream) return { text: `${stream.is_mature ? "🔞" : "🔴"} Live (${stream.viewers}) ${stream.categories[0]?.name} - "${stream.session_title}" m3u8: ${await shortLink(data.playback_url)}`, reply: true }

            if (!stream) return { text: "no active stream found", reply: true }

            const { body: vod } = await kick(`video/${stream.video.uuid}`)
            return { text: `Offline, last vod: (${utils.humanize(stream.duration, true)}) ${stream.categories[0]?.name} - "${stream.session_title}" m3u8: ${vod.source}` }
        }
