...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        if (values.json)
            ...
            if (data.livestream) return { text: `${stream.is_mature ? "🔞" : "🔴"} Live (${stream.viewers}) ${stream.categories[0]?.name} - "${stream.session_title}" ${data.playback_url.split("?")[0]}`, reply: true }
