...
module.exports = {
    name: 'toptracks',
    description: "Top 5 listened tracks on Spotify",
    cooldown: 7,
    aliases: ['top-tracks', 'topsongs'],
    usage: '[username]',
    async execute(client, msg, utils) {
        ...
        const { body: data } = await got("https://api.spotify.com/v1/me/top/tracks?limit=10", {
            responseType: 'json',
            headers: {
        ...
        const topTracks = data.items.map(i => `${i.artists.map(artist => artist.name).join(', ')} - ${i.name}`)
        return { text: `${user.id === msg.user.id ? 'your' : `${user.login}'s`} top listened tracks: ${topTracks.join(' \u{2022} ')}`, reply: true }
    },
};
