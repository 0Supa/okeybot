...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        return {
            text: `MrDestructoid 🏓 BOT Uptime: ${utils.humanize(client.connectedAt)}, DB Uptime: ${utils.humanizeMS(promises[1][0].variable_value * 1000)}, QPS: ${(promises[1][0].variable_value / promises[0][0].variable_value).toFixed(3)}, Redis Keys: ${promises[3]}, Active Channels: ${Object.keys(client.userStateTracker.channelStates).length}, Issued Commands: ${client.issuedCommands}, RAM: ${Math.round(process.memoryUsage().rss / 1024 / 1024)}mb, TMI: ${latency}ms, PubSub connections: ${pubsub.connections.length}, Banphrase API: ${banphraseStatus}`,
            reply: true
        }
    },
};