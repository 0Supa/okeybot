...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        const redisKeys = await utils.redis.dbsize()
        ...
        return {
            text: `MrDestructoid 🏓 BOT Uptime: ${utils.humanize(client.connectedAt)}, DB Uptime: ${utils.humanizeMS(dbUptime * 1000)}, QPS: ${(dbQueries / dbUptime).toFixed(3)}, Redis Keys: ${redisKeys}, Active Channels: ${Object.keys(client.userStateTracker.channelStates).length}, Issued Commands: ${client.issuedCommands}, RAM: ${Math.round(process.memoryUsage().rss / 1024 / 1024)}mb, TMI: ${latency}ms, PubSub connections: ${pubsub.connections.length}, Banphrase API: ${banphraseStatus ?? "not active"}`,
            reply: true
        }
    },
};