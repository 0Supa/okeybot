...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        return { text: `${b}${song.is_playing ? "▶️" : "⏸️"}(${format(song.progress_ms)}/${format(song.item.duration_ms)}) ${song.item.name} - by: ${song.item.artists.map(artist => artist.name).join(', ')} 🔗 ${song.item.external_urls.spotify.substring(9)}`, reply: true }
    },
};