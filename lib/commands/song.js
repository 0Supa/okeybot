...
const { humanizeSong } = require('../utils/utils.js')
...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        return { text: `${song.is_playing ? "▶️" : "⏸️"} ${humanizeSong(song.progress_ms)}/${humanizeSong(song.item.duration_ms)} | ${song.item.name}, by: ${song.item.artists.map(artist => artist.name).join(' & ')}`, reply: true }
    },
};