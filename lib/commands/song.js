...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        return { text: `${b}${song.is_playing ? "▶️" : "⏸️"}[${format(song.progress_ms)}/${format(song.item.duration_ms)}] ${song.item.name} • ${song.item.artists.map(artist => artist.name).join(', ')} • ${await shortLink(song.item.external_urls.spotify) ?? "N/A"}`, reply: true }
    },
};
