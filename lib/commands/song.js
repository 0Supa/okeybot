...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        return { text: `${song.is_playing ? "▶️" : "⏸️"} ${songHumanize(song.progress_ms)}/${songHumanize(song.item.duration_ms)} | ${song.item.name}, by: ${song.artists.map(artist => artist.name).join(' & ')}`, reply: true }
    },
};