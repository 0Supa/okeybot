...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        if (user.id !== msg.user.id) {
            a = `${user.login} is`
            b = `${user.login} is currently listening to: `
        }
        ...
        return { text: `${b}${song.is_playing ? "▶️" : "⏸️"}(${format(song.progress_ms)}/${format(song.item.duration_ms)}) ${song.item.name} - by: ${song.item.artists.map(artist => artist.name).join(', ')} 🔗 ${song.item.external_urls.spotify.substring(8)}`, reply: true }
    },
};