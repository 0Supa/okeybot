...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        return {
            text: `${user.id !== msg.user.id ? `${user.login} ${song.is_playing ? 'is currently' : 'was'} listening to: ` : ''}${song.is_playing ? "▶️" : "⏸️"}[${format(song.progress_ms)}/${format(song.item.duration_ms)}] ${song.item.name} • ${song.item.artists.map(artist => artist.name).join(', ')}${song.item.preview_url ? ` | ${await shortLink(song.item.preview_url.split('?')[0])}` : ""}`, reply: true
        }
    },
};
