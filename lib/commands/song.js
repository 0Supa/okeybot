...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        if (!song) return { text: `${user.id === msg.user.id ? 'you are' : `${utils.antiPing(user.login)} is`} not listening anything on Spotify`, reply: true }
        ...
        return {
            text: `${user.id !== msg.user.id ? `${utils.antiPing(user.login)} ${song.is_playing ? 'is currently' : 'was'} listening to: ` : ''}${song.is_playing ? "▶️" : "⏸️"}[${format(song.progress_ms)}/${format(song.item.duration_ms)}] ${song.item.name} • ${song.item.artists.map(artist => artist.name).join(', ')}${shortSongLink ? ` | ${shortSongLink}` : ""}`, reply: true
        }
    },
};
