...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        if (!auth) return { text: `you don't have Spotify linked with your Twitch account, you can login here: ${config.website.url}/spotify`, reply: true }
        ...
        if (msg.commandName === 'preview')
            return { text: `(${format(song.item.duration_ms)}) ${song.item.name} | Preview: ${song.item.preview_url ? song.item.preview_url.split('?')[0].substring(8) : "N/A"}`, reply: true }

        return { text: `${b}${song.is_playing ? "▶️" : "⏸️"}(${format(song.progress_ms)}/${format(song.item.duration_ms)}) ${song.item.name} - by: ${song.item.artists.map(artist => artist.name).join(', ')} ${await shortLink(song.item.external_urls.spotify) ?? "N/A"}`, reply: true }
    },
};
