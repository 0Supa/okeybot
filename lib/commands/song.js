...
async function getBest(arr) {
    for (data of arr) {
        const auth = await spotify.getToken(data.id)

        if (auth) {
            return { user: data, auth }
        } else if (data.forced) {
            return { error: "the targeted user doesn't have Spotify connected with their Twitch account" }
        }
    }
}
...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        const { user, auth, error } = await getBest(users)
