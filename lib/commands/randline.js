module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        if (!msg.args.length) {
            const query = await utils.query(`SELECT user_login, message AS text, timestamp FROM messages WHERE channel_id=? ORDER BY id DESC LIMIT 100000`, [msg.channel.id])
        ...
        const query = await utils.query(`SELECT user_login, message AS text, timestamp FROM messages WHERE channel_id=? AND user_login=? ORDER BY id DESC LIMIT 100000`, [msg.channel.id, user])
