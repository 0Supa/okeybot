module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        if (!msg.args.length) {
            ...
            const randID = utils.randArray(query)
            const message = await getMessage(randID)
        ...
        async function getMessage(id) {
            const message = await utils.query(`SELECT user_login, message AS text, timestamp FROM messages WHERE id=?`, [id])
            return message
        }
    },
};