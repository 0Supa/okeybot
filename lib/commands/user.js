    ...
    async execute(client, msg, utils) {
        ...
        switch (msg.commandName) {
            case "user": {
                ...
                return { text: `${user.banned ? "Banned - " : ""}${user.id} - ${user.login}, color: ${user.chatColor || "(none)"}, badges: ${user.badge.length ? user.badge.map(badge => badge.title).join(' | ') : "(none)"}, age: ${utils.humanize(user.createdAt)}${flags.length ? `, [${flags.join(' - ')}]` : ""}`, reply: true }
            }
            case "uid": {
            ...
            case "isbot": {
                return { text: `${user.displayName} | Verified Bot: ${user.roles.bot ? "TRUE" : "false"}`, reply: true }
            }
            case "partner": {
