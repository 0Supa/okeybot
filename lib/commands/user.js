...
module.exports = {
    ...
    cooldown: 3,
    aliases: ['uid', 'staff', 'isbot', 'partner', 'affiliate', 'banned', 'age'],
    async execute(client, msg, utils) {
        ...
        switch (msg.commandName) {
            case "user": {
                let flags = []
                const lg = user.settings.preferredLanguageTag
                const flag = utils.flag(user.settings.preferredLanguageTag)
                ...
                return { text: `${user.banned ? "Banned - " : ""}${lg ? (flag ? `${flag} ` : `${lg} `) : ""}${user.id} - ${user.login}, color: ${user.chatColor || "(none)"}, badges: ${user.badge.length ? user.badge.map(badge => badge.title).join(' | ') : "(none)"}, age: ${utils.humanize(user.createdAt)}${flags.length ? `, [${flags.join(' - ')}]` : ""}`, reply: true }
            }
            case "uid": {
            ...
            case "banned": {
                return { text: `${user.displayName} | Banned: ${user.banned ? "TRUE" : "false"}`, reply: true }
            }
            case "age": {
