...
module.exports = {
    ...
    cooldown: 3,
    aliases: ['uid', 'staff', 'isbot', 'partner', 'affiliate', 'banned', 'age', 'color', 'bio', 'banner'],
    ...
    async execute(client, msg, utils) {
        ...
        switch (msg.commandName) {
            ...
            case "bio":
                return { text: `${userTag} • Bio: ${user.bio ? `"${user.bio}"` : "N/A"}` }

            case "banner":
                return { text: `${userTag} • Profile Banner: ${user.banner ?? "(none)"}` }
        }
    },
};
