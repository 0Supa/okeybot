    ...
    async execute(client, msg, utils) {
        ...
        switch (msg.commandName) {
            case "user": {
                ...
                return { text: `${user.banned ? `♿ Banned (${user.banReason}) - ` : ""}${user.id} - ${user.login}, color: ${user.chatColor || "(none)"}, badges: ${user.badges.length ? user.badge.map(badge => badge.title).join(' | ') : "(none)"}, age: ${utils.humanize(user.createdAt)}${flags.length ? `, [${flags.join(' - ')}]` : ""}`, reply: true }
            }
