...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        if (banData.notFound) return { text: `user is not tracked by StreamerBans${user.banned ? ", but I can tell you that this user is banned" : ", but I can tell you that this user is not banned"}`, reply: true }
        ...
        const longestBan = Math.max.apply(null, userData.bans.map(ban => new Date(ban.ended_at || Date.now()) - new Date(ban.created_at)))
