...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        const longestBan = userData.bans.length === 1 ? new Date(bans[0].ended_at) - new Date(bans[0].created_at) : Math.max.apply(null, userData.bans.map(ban => new Date(ban.ended_at) - new Date(ban.created_at)))
