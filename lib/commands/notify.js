...
module.exports = {
    name: 'notify',
    description: "You'll get mentioned in a message when the **current channel** stream title/game changes or the stream goes online/offline 🛎",
    aliases: ['notifyme', 'unnotify'],
    cooldown: 5,
    async execute(client, msg, utils) {
        const notifyData = await utils.query(`SELECT COUNT(id) AS query FROM notify_channels WHERE user_id=?`, [msg.channel.id])
        ...
        if (msg.commandName === 'unnotify') {
            const data = (await utils.query(`SELECT id, user_login FROM notify_users WHERE channel_id=? AND user_id=? LIMIT 1`, [msg.channel.id, msg.user.id]))[0]
            if (data) {
                await utils.query(`DELETE FROM notify_users WHERE id=?`, [data.id])
            ...
            const data = (await utils.query(`SELECT user_login FROM notify_users WHERE channel_id=? AND user_id=? LIMIT 1`, [msg.channel.id, msg.user.id]))[0]
            if (data) {
                if (data.user_login !== msg.user.login) {
                    await utils.query(`UPDATE notify_users SET user_login=? WHERE channel_id=? AND user_id=? LIMIT 1`, [msg.user.login, msg.channel.id, msg.user.id])
            ...
            await utils.query(`INSERT INTO notify_users (channel_id, channel_login, user_id, user_login) VALUES (?, ?, ?, ?)`, [msg.channel.id, msg.channel.login, msg.user.id, msg.user.login])
