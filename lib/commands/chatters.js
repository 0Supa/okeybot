...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        const list = `chatters count: ${chatters.count}
        broadcasters: ${chatters['broadcasters'].map(u => u.login).join(' ') || 'N/A'}
        moderators: ${chatters['moderators'].map(u => u.login).join(' ') || 'N/A'}
        vips: ${chatters['vips'].map(u => u.login).join(' ') || 'N/A'}`

        return { text: `${chatters.count} chatters (${chatters.moderators.length} Moderators, ${chatters.vips.length} VIPs): ${await paste(list)}`, reply: true }
    },
};
