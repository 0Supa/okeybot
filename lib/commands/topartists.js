...
module.exports = {
    name: 'topartists',
    description: "Top 3 listened artists on Spotify",
    cooldown: 7,
    aliases: ['top-artists'],
    usage: '[username]',
    async execute(client, msg, utils) {
        ...
        const { body: data } = await got("https://api.spotify.com/v1/me/top/artists?limit=10", {
            responseType: 'json',
            headers: {
        ...
        const topArtists = data.items.map(i => `${i.name}${i.genres.length ? ` (${i.genres[0]})` : ''}`)
        return { text: `${user.id === msg.user.id ? 'your' : `${user.login}'s`} top listened artists: ${topArtists.join(' \u{2022} ')}`, reply: true }
    },
};
