...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        if (!stream) {
            ...
            if (!lastStream) return { text: `${utils.antiPing(user.login)} never streamed`, reply: true }

            return { text: `${utils.antiPing(user.login)} last streamed ${utils.humanize(lastStream.startedAt)} ago, the title was: ${lastStream.title || "(none)"}`, reply: true }
        }
