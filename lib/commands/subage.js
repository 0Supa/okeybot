...
module.exports = {
	...
	async execute(client, msg, utils) {
		...
		if (subs.cumulative.start == null)
			return {
				text: `${utils.antiPing(user)} don't have a subscription to ${msg.args[1]}`
			}
		if (subs.subscribed == false && subs.cumulative.start != null)
			return {
				text: `${utils.antiPing(user)} don't have a subscription to ${msg.args[1]} But the User has been subscribed for ${subs.cumulative.months} months previously. [Sub expired ${utils.humanize(subs.cumulative.end)} ago]`
			}
		if (subs.meta.gift) {
			return {
