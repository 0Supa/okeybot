...
module.exports = {
    ...
    async execute(client, msg, utils) {
        ...
        return { text: `${user.banned ? "Banned - " : ""}${user.id} - ${user.login}, color: ${user.chatColor || "(none)"}, badges: ${user.badge.length ? user.badge.map(badge => badge.title).join(' | ') : "(none)"}${flags.length ? `, [${flags.join(' - ')}]` : ""}`, reply: true }
    },
};