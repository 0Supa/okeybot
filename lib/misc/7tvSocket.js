    ...
    ws.addEventListener('message', async ({ data }) => {
        ...
        switch (msg.action) {
            ...
            case 'update': {
                ...
                if (payload.action === "UPDATE") client.say(payload.channel, `[7TV] ${payload.actor || "(unknown)"} aliased the emote "${payload.emote.name}" in "${payload.name}"`)
                else client.say(payload.channel, `[7TV] ${payload.actor || "(unknown)"} ${payload.action === 'ADD' ? "added" : "removed"} the emote "${payload.name}"`)
                break;
            }
