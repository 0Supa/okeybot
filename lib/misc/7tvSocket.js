...
const ignoredUsers = [config.bot.login, 'dontaddthisbot', 'gempbot']
    ...
    ws.addEventListener('message', async ({ data }) => {
        ...
        switch (msg.action) {
            ...
            case 'update': {
                ...
                if (ignoredUsers.includes(payload.actor.toLowerCase())) return
                ...
                if (payload.action === "UPDATE") client.say(payload.channel, `7tvM • ${userString} aliased the emote "${payload.emote.name}" in "${payload.name}"`)
                else client.say(payload.channel, `7tvM • ${userString} ${payload.action === 'ADD' ? "added" : "removed"} the emote "${payload.name}"`)
                break;
            }
