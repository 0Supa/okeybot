...
const ignoredUsers = ['supibot', 'streamelements', 'nightbot', 'fossabot', 'ksyncbot', 'apulxd', 'moobot', 'vjbotardo', 'snappingbot'] // FDM
...
module.exports = {
    handle: async function (msg) {
        if (msg.user.id !== config.bot.userId) {
            ...
            if (command && !cooldown.has(`${command.name}-${msg.user.id}`) && msg.text.startsWith(msg.prefix) && !ignoredUsers.includes(msg.user.login)) {
                ...
                if (botRequires && msg.channel.id !== config.bot.userId) {
                    ...
                    switch (botRequires) {
                        case "vip": {
                            cooldown.set(`${command.name}-${msg.user.id}`, 1000)
                ...
                if (access && msg.user.id !== config.owner.userId) {
                    ...
                    switch (access) {
                        case "vip": {
                            cooldown.set(`${command.name}-${msg.user.id}`, 3000)
                            if (!vip && !mod && !broadcaster) return msg.send(`${msg.user.name}, you need to be a vip to use this command`)
                        }; break;
                        case "mod": {
                            cooldown.set(`${command.name}-${msg.user.id}`, 3000)
