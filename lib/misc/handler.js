        ...
        if (msg.user.id !== config.bot.userId) {
            ...
            if (command) {
                ...
                try {
                    ...
                    const query = await utils.query(`INSERT INTO errors (type, data, error) VALUES (?); SELECT LAST_INSERT_ID() AS id;`, ['Command', errorContext, err])
                    msg.send(`${msg.user.name}, the command execution resulted in an error :/ - It was logged for a review with ID: ${query[0].id}`);
                }
            }
