...
module.exports = {
    handle: async function (msg) {
        if (msg.user.id !== process.env.userid) {
            ...
            if (command && !cooldown.has(`${command.name} ${msg.user.id}`) && msg.text.startsWith(msg.prefix)) {
                ...
                try {
                    ...
                    if (command.cooldown && msg.user.login !== process.env.owner_login) {
                        ...
                        }, result?.error ? 1000 : command.cooldown * 1000);
                    }
            ...
            if (msg.user.login === 'kazimir33') client.say(msg.channel.login, '@kazimir33 KEKW')
            if (msg.channel.login === 'supinic' && msg.user.login === 'supibot' && msg.text === 'ppCircle') client.say('supinic', 'ppAutismo')
        }

        if (msg.channel.query.logging) await utils.query(`INSERT INTO messages (channel_id, channel_login, user_id, user_login, message, timestamp) VALUES (?, ?, ?, ?, ?, ?)`, [msg.channel.id, msg.channel.login, msg.user.id, msg.user.login, msg.text, new Date(msg.timestamp)])
    }
};