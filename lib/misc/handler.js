...
module.exports = {
    handle: async function (msg) {
        if (msg.user.id !== config.bot.userId) {
            msg.prefix = msg.channel.query.prefix.toLowerCase() ?? config.bot.defaultPrefix
            ...
            if (command) {
                ...
                if (cooldown.has(cooldownKey) || !msg.text.startsWith(msg.prefix) || client.ignoredUsers.has(msg.user.id)) return
