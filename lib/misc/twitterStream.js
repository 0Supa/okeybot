...
exports.init = () => {
    ...
    stream.on('data', async data => {
        if (data.toString() === '\r\n') {  // keep-alive signal (sent at least every 20s)
            lastHeartbeat = Date.now()
        ...
        try {
            ...
            for (uid of followers) {
                ...
                let text = `VisLaud New tweet from ${author.name}: ${utils.unescapeHTML(msg.data.text)}`.replace(/\n|\r/g, ' ')
    ...
    stream.once('retry', async (_, error) => {
        if (retryCount > 3) await utils.sleep(300000)
