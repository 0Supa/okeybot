...
exports.init = () => {
    ...
    stream.on('data', async data => {
        ...
        try {
            const tweet = await twitter.get(`tweets/${msg.data.id}?expansions=author_id`)
            const followers = await utils.redis.smembers(`ob:twitter:followers:${user}`)

            const author = tweet.users.map(u => u.name).join(' and ')
            for (uid of followers) {
                const channel = await utils.getChannel(uid)
                let msg = `VisLaud New tweet from ${author}: ${tweet.data.text}`
