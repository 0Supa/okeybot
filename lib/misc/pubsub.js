...
ps.addEventListener('open', async () => {
    ...
    const topics = [];
    const channels = await utils.query('SELECT user_id, redeems FROM notify_data')
    for (let channel of channels) {
        topics.push(`video-playback-by-id.${channel.user_id}`, `broadcast-settings-update.${channel.user_id}`)
        if (channel.redeems) topics.push(`community-points-channel-v1.${channel.user_id}`)
    }
    ...
    switch (msg.type) {
        ...
        case 'MESSAGE':
            ...
            switch (msgData.type) {
                case 'stream-up':
                case 'stream-down':
                    handleWSMsg({ channel_id: msgTopic.replace('video-playback-by-id.', ''), type: msgData.type });
                    break;
                case 'viewcount':
                case 'commercial':
                    break;
...
const handleWSMsg = async (msg = {}) => {
    ...
    switch (msg.type) {
        case 'stream-up': {
            const data = (await utils.query(`SELECT live, login FROM notify_data WHERE user_id=?`, [msg.channel_id]))[0]
            if (data.live) return;
            await utils.query(`UPDATE notify_data SET live=? WHERE user_id=?`, [true, msg.channel_id])
            utils.notify(data.login, 'online')
        }
            break;
        case 'stream-down': {
            const data = (await utils.query(`SELECT live, login FROM notify_data WHERE user_id=?`, [msg.channel_id]))[0]
            if (!data.live) return;
            await utils.query(`UPDATE notify_data SET live=? WHERE user_id=?`, [false, msg.channel_id])
            utils.notify(data.login, 'offline')
        }
            break;
