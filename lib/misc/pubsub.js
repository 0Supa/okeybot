...
exports.init = async () => {
    ...
    listen((await utils.query('SELECT login, platform_id AS id FROM channels WHERE parted=?', [false])), ['polls', 'predictions-channel-v1']) // polls & predictions alerts
...
exports.createListener = (channel, sub) => {
    const nonce = crypto.randomBytes(20).toString('hex').slice(-8);
    const c = connections.find(({ topics }) => topics.length < 50)

    if (c) {
        const message = {
            'data': {
                'auth_token': `Bearer ${config.auth.twitch.password}`,
