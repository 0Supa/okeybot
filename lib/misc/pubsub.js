...
const handleWSMsg = async (msg = {}) => {
    ...
    switch (msg.type) {
        ...
        case 'POLL_COMPLETE': {
            ...
            const mostVotes = Math.max(...msg.data.poll.choices.map(choice => choice.votes.total))
            const winners = msg.data.poll.choices.filter(choice => choice.votes.total === mostVotes).map(choice => choice.title)

            client.say(channel.login, `Poll Complete VisLaud 👉 ${msg.data.poll.title} 🏆: ${winners.join(" | ")}`)
            break;
        }
