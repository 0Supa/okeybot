            ...
            try {
                switch (data[0].app) {
                    case "bttv": {
                        ...
                        client.say(data[0].channel_login, `[BTTV] VisLaud 👉 ${redemption.user.display_name} successfully added emote ${addedEmote} and removed ${removedEmote}`)
                        break;
                    }
                    case "7tv": {
                        let stvID
                        ...
                        await utils.query(`UPDATE emotes SET emote_id=? WHERE channel_id=? AND reward_title=?`, [parsedInput[1], redemption.channel_id, redemption.reward.title])
                        client.say(data[0].channel_login, `[7TV] VisLaud 👉 ${redemption.user.display_name} successfully added emote ${addedEmote} and removed ${removedEmote}`)
                        break;
                    }
                }
            } catch (err) {
