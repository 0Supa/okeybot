    ...
    switch (msg.type) {
        ...
        case 'reward-redeemed': {
            ...
            try {
                ...
                await utils.query(`UPDATE emotes SET emote_id=? WHERE channel_id=?`, [parsedInput[1], redemption.channel_id])
                client.say(data[0].channel_login, `VisLaud ${redemption.user.display_name} successfully added emote ${addedEmote} and removed ${removedEmote}`)
            } catch (err) {
                const { statusCode } = await got.post('https://gql.twitch.tv/gql', {
                    headers: {
                ...
                client.say(data[0].channel_login, `${redemption.user.display_name}, monkaS error: ${err} | ${(statusCode < 200 || statusCode > 299) ?
                    'MODS please refund her/his points' : 'I successfully refunded your points 👍'}`)
            }
            break;
        }
