...
const handleWSMsg = async (msg = {}) => {
    ...
    switch (msg.type) {
        ...
        case 'reward-redeemed': {
            ...
            const data = await utils.query('SELECT channel_login, app_userid, emote_id, reward_title FROM emotes WHERE channel_id=?', redemption.channel_id)
            ...
            try {
                if (!data.app_userid) {
                    ...
                    await utils.query(`UPDATE emotes SET app_userid=? WHERE channel_id=?`, [bttvID, redemption.channel_id])
                } else {
