...
exports.init = async () => {
    listen((await utils.query('SELECT user_id AS id, login FROM notify_channels')), ['broadcast-settings-update']) // notify command

    listen((await utils.query('SELECT channel_id AS id, channel_login AS login FROM emote_rewards')), ['community-points-channel-v1']) // emote redeems

    listen((await utils.query('SELECT login, platform_id AS id FROM channels')), ['video-playback-by-id', 'chatrooms-user-v1']) // global subs

    // KKona
    // listen([{ login: 'chimichanga', id: '227322800' }], ['crowd-chant-channel-v1'])

    const unbanNotifs = [{ login: "8supa", id: "675052240" }, { login: "kazimir33", id: "108311159" }]
    ...
    ws.addEventListener('message', ({ data }) => {
        ...
        switch (msg.type) {
            ...
            case 'MESSAGE':
                ...
                handleWSMsg({ channelID: msgData.data?.channel_id || msgTopic.split('.').pop(), ...msgData })
                break;

            case 'RECONNECT':
                logger.info(`[${id}] PubSub server sent a reconnect message. restarting the socket`);
                if (ws.readyState === 1) ws.reconnect();
                break;
