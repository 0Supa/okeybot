...
exports.init = async () => {
    ...
    const unbanNotifs = [{ login: "8supa", id: "675052240" }]
    listen(unbanNotifs, ['channel-unban-requests'])
...
const handleWSMsg = async (msg = {}) => {
    ...
    switch (msg.type) {
        ...
        case 'create_unban_request': {
            const channel = await utils.getChannel(msg.channelID)
            if (!channel) return
            client.say(channel.login, `New unban request from "${msg.data.requester_login}" MODS`)
            break;
        }
