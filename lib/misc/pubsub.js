...
exports.init = async () => {
    listen((await utils.query('SELECT user_id AS id, login FROM notify_channels')), ['video-playback-by-id', 'broadcast-settings-update']) // notify command
    ...
    listen((await utils.query('SELECT login, platform_id AS id FROM channels')), ['polls', 'predictions-channel-v1', 'chatrooms-user-v1']) // global subs
